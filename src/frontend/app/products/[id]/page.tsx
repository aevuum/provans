...
export default function ProductDetailPage() {
  ...
  const { items: favorites } = useAppSelector((state: RootState) => state.favorites);
  const cartItems = useAppSelector((state: RootState) => state.cart.items);
  const isFavorite = favorites.some((item: Product) => item.id === product?.id);
  const inCart = cartItems.some((c: { id: number }) => c.id === product?.id);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <button
            onClick={() => router.push('/catalog/all-category')}
