...
export async function PATCH(
  ...
  try {
    ...
    // Обновляем товар
    const product = await tryPrisma(() => prisma.product.update({
      where: { id: productId },
      data: updateData
    }), { timeoutMs: 1500, retries: 1 });

    // tryPrisma returns `T | undefined` on error; some prisma calls can also
    // (very rarely) return `null` for not-found — guard both cases so TypeScript
    // knows `product` is non-null below.
    if (product === undefined) {
      console.error('products/[id].PATCH: DB unreachable when updating product');
      return NextResponse.json({ error: 'Service unavailable' }, { status: 503 });
    }

    if (product === null) {
      // If update somehow returned null, treat as not found.
      return NextResponse.json({ error: 'Товар не найден' }, { status: 404 });
    }

    // product may be a plain object from Prisma or a proxied value; cast to any
    const prodAny: any = product;
    const createdAtStr = prodAny.createdAt ? new Date(prodAny.createdAt).toISOString() : null;
    const updatedAtStr = prodAny.updatedAt ? new Date(prodAny.updatedAt).toISOString() : null;
