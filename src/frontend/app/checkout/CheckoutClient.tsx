...
export default function CheckoutPage() {
  ...
  const subtotal = cart.reduce((sum: number, item: CartItem) => sum + item.price * (item.count || 1), 0);
                ...
                return (
                  <div key={item.id} className="flex items-center space-x-3">
                    <div className="w-12 h-12 bg-gray-100 rounded overflow-hidden flex-shrink-0">
                      <LazyImage src={mainImage} alt={item.title} width={48} height={48} className="w-full h-full object-cover" />
                    </div>
