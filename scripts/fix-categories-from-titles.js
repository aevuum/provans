...
function load() {
  const p = path.join(process.cwd(), 'new-product.json');
...
function guessCategory(title) {
  ...
  if (/рамк|фоторамк|фото рамк|фото-рамк/.test(t)) add('frames', 10);
...
(function main() {
  ...
  const ts = Date.now();
  const backup = path.join(process.cwd(), `new-product.backup.${ts}.json`);
