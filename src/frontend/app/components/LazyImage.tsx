...
export function LazyImage({ 
  ...
  // Прямой путь без прокси /api/image (удаляем тяжёлый серверный обработчик)
  // Если есть спецсимволы — браузер сам корректно запросит статический файл из /public
  const [imageSrc, setImageSrc] = useState(src);
  useEffect(() => {
    let active = true;
    resolveImageWithManifest(src).then(r => { if (active) setImageSrc(r); });
    return () => { active = false; };
  }, [src]);
