...
export async function POST(
  ...
  const current = await tryPrisma(() => prisma.product.findUnique({ where: { id: productId } })) as unknown as ProductMinimal | null | undefined;
    if (!current) {
      return NextResponse.json({ error: 'Товар не найден или DB unavailable' }, { status: 404 });
    }
  ...
  const hasAnyImage = (!!current.image && !isPlaceholder(current.image) && current.image.trim() !== '') || images.some((i: unknown) => !!i && !isPlaceholder(i as string) && (i as string).trim() !== '');
  ...
  category: cat || null, // обновляем категорию если задана
    ...
    try {
      const confirmed = await tryPrisma(() => prisma.product.findMany({ where: { isConfirmed: true } }));
      if (Array.isArray(confirmed)) {
        const payload = {
          products: confirmed.map((p: ProductMinimal) => ({
            title: p.title,
        ...
        console.warn('Skipping archive sync — DB unavailable');
      }
    } catch (e) {
