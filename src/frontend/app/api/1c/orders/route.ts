...
import { C1Order } from '@/lib/1c-config';
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!Array.isArray(orders)) {
      console.warn('[1c/orders] prisma returned undefined or DB unreachable');
      return NextResponse.json({ status: 'success', data: [], count: 0, syncTime: new Date().toISOString() });
    }

  // Преобразуем в формат для 1С
  const c1Orders: C1Order[] = orders.map((order: any) => ({
  ...
  try {
    ...
    for (const order of orders) {
      try {
        ...
        await tryPrisma(() => prisma.order.update({
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const newOrder = await tryPrisma(() => prisma.order.create({
      data: {
        customerName: order.customerName,
        customerEmail: order.customerEmail,
        customerPhone: order.customerPhone || '',
    ...
    const createdOrder = newOrder as { id: number } | null | undefined;
    if (createdOrder === undefined) {
      console.warn('[1c/orders] prisma returned undefined (DB unreachable) when creating order');
      return NextResponse.json({ error: 'Service unavailable' }, { status: 503 });
    }
    if (!createdOrder) {
      console.warn('[1c/orders] failed to create order — prisma returned null');
