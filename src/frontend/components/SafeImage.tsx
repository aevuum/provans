...
const SafeImage = memo<SafeImageProps>(({
  ...
  useEffect(() => {
    let ok = true;
    resolveImageWithManifest(src).then(r => { if (ok) setImageSrc(r); });
    return () => { ok = false; };
  }, [src]);
  ...
  const imageProps = {
    ...
    // Use next/image's onLoadingComplete to detect when the image finished loading.
    // next/image may not forward the DOM `onLoad` handler in some versions/configs,
    // so onLoadingComplete is the reliable callback here.
  ...
  return (
    <div
      className={`${fill ? 'absolute inset-0' : `relative ${(fill || (!width && !height)) ? 'w-full h-full' : ''}`} ${onClick ? 'cursor-pointer' : ''}`}
      onClick={handleClick}
    >
      {isLoading && (
        <div
