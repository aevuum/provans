...
export function generateCatalogBreadcrumbs(category?: string, productTitle?: string): BreadcrumbItem[] {
  ...
  // Компонент Breadcrumbs уже рендерит иконку "Главная" отдельно,
  // поэтому здесь добавляем только последующие элементы (Каталог/Акции/Новинки и т.д.).
  ...
  breadcrumbs.push({ name: 'Каталог', href: '/catalog/all-category' });
  ...
  'all': { name: 'Все товары', href: '/catalog/all-category' },
