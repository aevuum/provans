...
export async function POST(req: NextRequest) {
  try {
    ...
    const product = await tryPrisma(() => prisma.product.create({
      data: {
        title: title.trim(),
    ...
    if (!product) {
      console.error('Create product failed: DB unavailable');
...
export async function GET(req: NextRequest) {
  try {
    ...
    const total = await tryPrisma(() => prisma.product.count({ where }));
